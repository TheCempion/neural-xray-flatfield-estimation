...
from utils import gpu_has_80GB
...
def main(config: ConfigParser):
    ...
    if gpu_has_80GB():
        ...
        if config["trainer"]["accum_batches"] <= 16:
            # values larger 16 indicate, that the total accumulated batchsize should be greater
            config["trainer"]["accum_batches"] = 16
        config["data_loader"]["args"]["batch_size_valid"] = 4
        if config.get("valid_data_loader_dataset_args", None) is not None:
            config["valid_data_loader_dataset_args"]["batch_size"] = 4
    ...
    optimizer, lr_scheduler = config.init_optim_lr_scheduler(model)
    logger.debug(f"{optimizer=}")
    logger.debug(f"{lr_scheduler=}")
